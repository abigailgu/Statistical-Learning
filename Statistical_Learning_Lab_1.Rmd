...
dep_delay <- as.data.frame(flights) %>% filter(origin == 'EWR') %>% group_by(dest) %>%
  count()
colnames(dep_delay) <-c("faa", "amount")

dep_delay2 <- as.data.frame(flights) %>% filter(origin == 'EWR') %>% group_by(dest) %>% filter(dep_delay >15) %>%   count()
